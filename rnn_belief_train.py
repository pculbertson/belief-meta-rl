...
encoder = LSTMEncoder(dim_obs,dim_actions,latent_dim,cov_type=encoder_cov_type,hs=encoder_hs,num_layers=encoder_num_layers).cuda()
...
for epoch in range(num_epochs):
    ...
    if epoch > 0:
        for step in range(num_train_steps):
            ...
            s,a_rb,r,sp = [torch.stack([torch.from_numpy(samp[k]).float().to(device) for samp in samps]) for k in ['o','a','r','op']]
            ...
            q_ins = torch.cat((s,a,r,sp),axis=2).cuda()
            q_outs, _ = encoder(q_ins)
            q_means = torch.squeeze(q_outs)[:,:,:latent_dim].unsqueeze(-1)
            ...
            net_ins = torch.cat((s,a,thetas.expand(batch_size,batch_length,latent_dim)),axis=2).to(device)
            ...
            t_mse = torch.nn.MSELoss()(t_means.to(device),sp)
            #log_rs = -torch.sum(torch.nn.MSELoss(reduction='none')(r_outs[:,:,0].to(device),torch.squeeze(r)),1)
            log_rs = -torch.norm(r_outs[:,:,0].to(device)-torch.squeeze(r),p=1)
    ...
    ep_step = 0
    latent_mean, latent_prec = torch.zeros(latent_dim).view(1,latent_dim,1).cuda(), torch.eye(latent_dim).view(1,latent_dim,latent_dim).cuda()
    hidden = None
    r =0.
    while not d and ep_step < ep_length:
        ...
        if epoch < random_episodes or epoch % episode_repeat == 0:
            ...
            print(r)
            if hidden is None:
                a = policy.get_action(s,hidden,latent_mean,latent_prec)
            else:
                a = policy.get_action(s,[h.expand(encoder_num_layers,num_traj,encoder_hs).contiguous() for h in hidden],latent_mean,latent_prec)
        ...
        codes = torch.squeeze(latent_mean.expand(test_batch_size,latent_dim,1).cuda() + torch.matmul(torch.inverse(latent_prec).expand(test_batch_size,latent_dim,latent_dim).cuda(),torch.randn(test_batch_size,latent_dim,1).cuda())).cuda()
        s_torch = torch.from_numpy(np.array(s)).float().expand(test_batch_size,dim_obs).cuda()
        a_torch = torch.from_numpy(np.array(a)).float().expand(test_batch_size,dim_actions).cuda()
        net_ins = torch.cat((s_torch,a_torch,codes),axis=1).to(device)
        
        sp_hats = trans_net(net_ins)[:,:dim_obs]
        r_hats = rew_net(net_ins)[:,0]
        
        t_err = torch.mean(torch.nn.MSELoss()(torch.from_numpy(sp).float().expand(test_batch_size,dim_obs).to(device),sp_hats))
        r_err = torch.mean(torch.nn.MSELoss()(torch.from_numpy(np.array([r])).float().expand(test_batch_size,1).to(device),r_hats))
        
        dyn_error += t_err
        rew_error += r_err
        
        q_in = torch.cat([torch.from_numpy(np.array(k)).float() for k in [s,a,[r],sp]]).view(1,1,-1).cuda()
        q_out, hidden = encoder(q_in,hidden)
        new_mean, new_prec = torch.squeeze(q_out)[:latent_dim].view(1,latent_dim,1).cuda(), torch.inverse(get_cov_mat(torch.squeeze(q_out)[latent_dim:],dim_obs,encoder_cov_type,device)).view(1,latent_dim,latent_dim).cuda()     
        latent_mean, latent_prec = gaussian_product_posterior(latent_mean.cuda(),latent_prec.cuda(),new_mean,new_prec)
