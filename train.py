...
policy = CrossEntropy(trans_net, rew_net, action_dist, num_traj, traj_length, num_iters, elite_frac, dim_obs, dim_actions, trans_cov_type, rew_cov, max_logvar)
...
for epoch in range(num_epochs):
    ...
    while not d:
        ...
        if discrete_actions:
            ...
            a = action_dist.pop(0).rsample().numpy()
        ...
        net_in = torch.cat((torch.from_numpy(s).float(),torch.from_numpy(a).float())).view(1,dim_obs+dim_actions)
        t_out = torch.squeeze(trans_net(net_in))
        t_mean, t_cov =  t_out[:dim_obs], t_out[dim_obs:]
        r_out = torch.squeeze(rew_net(net_in))
        
        dyn_error += torch.nn.MSELoss()(t_mean,torch.from_numpy(sp).float())
        rew_error += torch.nn.MSELoss()(r_out,torch.from_numpy(np.array(r)).float())
